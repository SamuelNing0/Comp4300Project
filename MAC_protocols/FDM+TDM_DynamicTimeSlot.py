...
packets_sent_time = [{},{},{}]
packets_sent = [0,0,0]
packets_sent_node = [[0,0,0],[0,0,0],[0,0,0]]
    ...
    while True:
        ...
        if chance_true(data_load_percent):
            with band.request() as req:
                yield req
                # update both band and node's total frames sent
                packets_sent[band_idx] += 1
                packets_sent_node[band_idx][node_num] += 1

                packets_sent_time[band_idx][env.now] = packets_sent[band_idx]
                packets_sent_time_node[band_idx][node_num][env.now] = packets_sent_node[band_idx][node_num]
...
def transmit(env, name, band, speed, data_load_percent, band_idx):
    # TDM slow duration in each band, speed dependent
    slot_duration = ((2*TOTAL_SPEED) / speed)+0.5
    yield env.timeout(0.1)

    min_load = min(data_load_percent)
    node_weight = [ math.ceil(load/min_load)  for load in data_load_percent]


    # Create and schedule transmissions for each node, based on the load of the node
    # each node is assigned with different number of time slots
    initial_slot_wait = 0
    for node_num in range(NUM_NODES_IN_BAND):

        # assign time slots based on node weight
        for i in range(node_weight[node_num]):
            env.process(node(env, f"{name}: Node {node_num+1} slot{i+1}", band, slot_duration, initial_slot_wait,
                             data_load_percent[node_num], band_idx, node_num))
            initial_slot_wait += slot_duration
...
plt.title(f'FDM+TDM(dynamic): Packet Transmission Over Time by Band. Total packets sent: {total_packets} '
